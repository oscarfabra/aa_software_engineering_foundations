...
# Accepts a sentence and a hash as arguments. The method returns a new sentence
...
# Accepts a hash, a proc, and a block. The method returns a new hash where 
# each key is the result of the original key when given to the block. Each 
...
# Accepts a string as an argument. The method returns an array containing the
...
# Accepts a string and a hash as arguments. The method returns a new string 
# where characters of the original string are replaced with the corresponding 
# values in the hash. If a character is not a key of the hash, then it should 
...
# Accepts a string as an argument. The string will be formatted so every letter
# is followed by a number. The method returns an "uncompressed" version of the
